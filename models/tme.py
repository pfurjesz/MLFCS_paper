...
def rmse(y, ŷ): return np.sqrt(mean_squared_error(y, ŷ))
def mae (y, ŷ): return mean_absolute_error(y, ŷ)
def mape(y, ŷ): return np.mean(np.abs((y-ŷ)/np.clip(y,1,None))) * 100
...
class _Expert(nn.Module):
    ...
    def forward(self,x):
        μ  = _bilinear(x,self.Lm,self.Rm,self.bm)
        lv = _bilinear(x,self.Lv,self.Rv,self.bv)
        σ2 = F.softplus(lv)+1e-5
        return μ,σ2
...
class TME:
    ...
    # ---------- loss
    def _nll(self, y, μ, σ2, w):
        lp = -0.5*(torch.log(2*np.pi*σ2) + (y.unsqueeze(1)-μ)**2/σ2)
        return -torch.logsumexp(torch.log(w.clamp(1e-4,1-1e-4))+lp,1).mean()

    # ---------- forward
    def _fwd(self,trx,lob):
        μ1,σ1 = self.net.ex1(trx)
        μ2,σ2 = self.net.ex2(lob)
        w     = self.net.gate(trx,lob)
        return torch.cat([μ1,μ2],1), torch.cat([σ1,σ2],1), w
    ...
    def train(self):
        ...
        for ep in range(1,epochs+1):
            ...
            for trx,lob,y,_ in self.train_dl:
                ...
                opt.zero_grad()
                μ,σ2,w=self._fwd(trx,lob)
                loss=self._nll(y,μ,σ2,w)
            ...
            with torch.no_grad():
                for trx,lob,y,_ in self.val_dl:
                    trx,lob,y=[t.double().to(DEVICE) for t in (trx,lob,y)]
                    μ,σ2,w=self._fwd(trx,lob)
                    tot+=self._nll(y,μ,σ2,w).item()
            val=tot/len(self.val_dl); self.va_curve.append(val)
            if val<best: best,best_state=val,self.net.state_dict()
            print(f"ep{ep:02d}  train {self.tr_curve[-1]:.4f}  val {val:.4f}")
        self.net.load_state_dict(best_state)

    # ---------- predict loader (returns raw log-space mean/var + mean_volume)
    @torch.no_grad()
    def _predict_loader(self,loader):
        self.net.eval()
        y_log, μ_log, σ2_log, mv = [],[],[],[]
        for trx,lob,y,m in loader:
            trx,lob,y=[t.double().to(DEVICE) for t in (trx,lob,y)]
            μ_k,σ2_k,w=self._fwd(trx,lob)
            μ = (w*μ_k).sum(1)
            σ2= (w*σ2_k).sum(1) + (w*((μ_k-μ[:,None])**2)).sum(1)
            y_log.append(y.cpu().numpy())
            μ_log.append(μ.cpu().numpy())
            σ2_log.append(σ2.cpu().numpy())
            mv.append(m.numpy())
        return map(np.concatenate,(y_log,μ_log,σ2_log,mv))

    # ---------- evaluate on test
    def evaluate(self):
        y_log, μ_log, σ2_log, mv = self._predict_loader(self.test_dl)

        y_true = np.exp(y_log) * mv
        y_pred = np.exp(μ_log) * mv
        sigma  = np.sqrt((np.exp(σ2_log)-1)*np.exp(2*μ_log+σ2_log)) * mv
    ...
    def _plot(self,y,ŷ,low,up):
        ...
        plt.fill_between(range(len(ŷ)),low,up,color="#9ecae1",alpha=.3,label="95 % band")
        plt.plot(y,label="true",lw=.7); plt.plot(ŷ,label="pred",lw=.7)
