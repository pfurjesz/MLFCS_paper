...
def train_ensemble_probs():
    ...
    file_name = f'ensemble_probs_10m/feat_checkpoint.pth'
    ...
    while not stop:
        if t % 100 == 0:
            print(f"Epoch {t + 1}\n-------------------------------")
        stop, best_val, counter = train_ensemble_probs_loop(train_dataloader,val_dataloader,train_x,train_y,val_x,val_y, model, nn.MSELoss(reduction='sum'), optimizer, t,
    ...
    print(f'NNLL = {NNLL((test_x*ens_probs).sum(1), torch.tensor(test_df["total_volume"].iloc[h:].to_numpy()),0)}')
...
def main(train=False):
    if train:
        ...
        # print(f'NNLL = {NNLL(predictions.mean(1),torch.tensor(test_df["total_volume"].iloc[h:].to_numpy())) + Lambda * reg_term/(2*20)}')
        print(
            f'NNLL = {NNLL((test_x * ens_probs).sum(1), torch.tensor(test_df["total_volume"].iloc[h:].to_numpy()), 0)}')
        print(
            f'IW = {torch.sqrt((ens_probs * torch.permute(IW, [1, 0])).sum(1) - torch.square((test_x * ens_probs).sum(1).detach())).mean()}')
